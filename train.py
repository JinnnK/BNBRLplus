	...
	for j in range(num_updates):
		...
		for step in range(algo_args.num_steps):
			...
			if env_name == 'CrowdSimPredRealGST-v0' and env_config.env.use_wrapper:
				...
				out_pred = rollouts_obs['spatial_edges'].to('cpu').numpy()
				if config.robot.belief != False:
					out_pred_ = rollouts_obs['belief_edges'].to('cpu').numpy()
					out_pred = np.concatenate((out_pred, out_pred_), axis=1)
