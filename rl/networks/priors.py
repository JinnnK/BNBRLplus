...
class chi_squared_prior(object):
    def __init__(self, df):
        self.df = df  # degrees of freedom

    def loglike(self, x):
        return -0.5 * (self.df * torch.log(0.5 * x)) - 0.5 * x - (self.df * torch.log(2) + torch.lgamma(self.df / 2))

class beta_prior(object):
    def __init__(self, alpha, beta):
        self.alpha = alpha
        self.beta = beta

    def loglike(self, x):
        return (self.alpha - 1) * torch.log(x) + (self.beta - 1) * torch.log(1 - x) - torch.lgamma(self.alpha) - torch.lgamma(self.beta) + torch.lgamma(self.alpha + self.beta)

class student_t_prior(object):
    def __init__(self, df):
        self.df = df  # degrees of freedom

    def loglike(self, x):
        return -0.5 * (self.df + 1) * torch.log(1 + x**2 / self.df) - 0.5 * torch.log(self.df) - torch.lgamma(0.5 * self.df)
