...
legends = ['BNDNN', 'BNBRL','BNBRL+', '', '', '']

# add any folder directories here!
log_list = [
    pd.read_csv("trained_models/BNDNN/progress.csv"),
    pd.read_csv("trained_models/BNBRL/progress.csv"),
    pd.read_csv("trained_models/BNBRL+/progress.csv"),
]
...
for i in range(len(graphDicts)):
    ...
    plt.title('Comparison of reward values')
