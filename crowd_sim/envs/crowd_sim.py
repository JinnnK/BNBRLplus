...
class CrowdSim(gym.Env):
    ...
    def configure(self, config):
        ...
        self.tracker = config.sim.tracker
        self.tracker_num = config.sim.tracker_num
        assert self.min_human_num >= self.tracker_num
    ...
    def update_tracker_goal(self, human):
    ...
    def detect_visible(self, state1, state2, robot1 = False, custom_fov=None, custom_sensor_range=None):
        ...
        if self.blink_toggle == False:   # normal FOV
            if self.blink_cnt >= self.blink_period:
                self.blink_toggle = True
                self.blink_cnt = 0.
        else:   # blink FOV
            if self.blink_cnt >= self.blink_time:
                self.blink_toggle = False
                self.blink_cnt = 0.
    ...
    def render(self, mode='human'):
        ...
        if self.robot_fov != 2 * np.pi:
            ...
            sector = Wedge(self.robot.get_position(),
                self.robot.sensor_range + self.robot.radius * 2, angle_degree + 180 - np.degrees(self.robot_fov) / 2, angle_degree + 180 + np.degrees(self.robot_fov) / 2, color='gray', fill=True)
