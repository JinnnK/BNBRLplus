...
class CrowdSimPredRealGST(CrowdSimPred):
    ...
    def set_robot(self, robot):
        ...
        if self.belief != False:
            d['belief_edges'] = gym.spaces.Box(low=-np.inf, high=np.inf,
                            shape=(self.config.sim.human_num + self.config.sim.human_num_range, self.spatial_edge_dim),
                            dtype=np.float32)

            d['belief_masks'] = gym.spaces.Box(low=-np.inf, high=np.inf,
                            shape=(self.config.sim.human_num + self.config.sim.human_num_range,),
                            dtype=np.bool)
    ...
    def generate_ob(self, reset, sort=False):
        ...
        if self.belief != False:
            ob['belief_edges'] = np.tile(parent_ob['belief_edges'], self.predict_steps+1)
            ob['belief_masks'] = parent_ob['belief_masks']
    ...
    def render(self, mode='human'):
        ...
        plt.fill([-20, 20, 20, -20], [-20, -20, 20, 20], "gray")

        if self.blink:
            if self.blink_toggle == False:
                if self.robot_fov != 0:
                    sensor = plt.Circle(self.robot.get_position(), self.robot.sensor_range + self.robot.radius * 2, fill=True, color='White')
                    ax.add_artist(sensor)
                    artists.append(sensor)
            else:
                pass
        else:
            if self.robot_fov != 0:
        ...
        if self.robot_fov != 2 * np.pi:
            angle = np.arctan2(self.robot.vy, self.robot.vx)
            angle_degree = np.degrees(angle)
            sector = Wedge(self.robot.get_position(),
                self.robot.sensor_range + self.robot.radius * 2, angle_degree + np.degrees(self.robot_fov) / 2, angle_degree - np.degrees(self.robot_fov) / 2, color='gray', fill=True)
            ax.add_artist(sector)
            artists.append(sector)
        ...
        if self.tracker:
            for i in range(self.tracker_num):
                human_circles[i].set_linewidth(4.5)
        ...
        for i in range(len(self.humans)):
            ...
            if self.gst_out_traj is not None:
                for j in range(self.predict_steps):
                    circle = plt.Circle(self.gst_out_traj[i, (2 * j + 2):(2 * j + 4)] + np.array([robotX, robotY]),
        ...
        # plot invisible human positions
        for i in range(len(self.humans), len(self.humans) * 2):
            # add future predicted positions of each human
            if self.gst_out_traj is not None:
                # the position of the invisible human
                circle = plt.Circle(self.gst_out_traj[i, :2] + np.array([robotX, robotY]),
                                        self.config.humans.radius + 0.2, fill=False, color='violet', linewidth=1.5)
                ax.add_artist(circle)
                artists.append(circle)
                # the trajectory of the invisible human
                for j in range(self.predict_steps):
                    circle = plt.Circle(self.gst_out_traj[i, (2 * j + 2):(2 * j + 4)] + np.array([robotX, robotY]),
                                        self.config.humans.radius, fill=False, color='xkcd:pale lavender', linewidth=1.5)
                    ax.add_artist(circle)
                    artists.append(circle)
