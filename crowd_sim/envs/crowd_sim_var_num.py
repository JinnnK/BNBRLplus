...
class CrowdSimVarNum(CrowdSim):
    ...
    def set_robot(self, robot):
        ...
        if self.belief != False:
            d['belief_edges'] = gym.spaces.Box(low=-np.inf, high=np.inf, shape=(self.max_human_num, 2), dtype=np.float32)
            d['belief_masks'] = gym.spaces.Box(low=-np.inf, high=np.inf, shape=(self.max_human_num,), dtype=np.bool)
    ...
    def generate_ob(self, reset, sort=False):
        ...
        if self.belief != False:
            ob['belief_edges'] = copy.deepcopy(ob['spatial_edges'])
            ob['belief_masks'] = copy.deepcopy(ob['visible_masks'])
    ...
    def step(self, action, update=True):
        ...
        # Update tracker's goal
        if self.tracker:
            for i in range(self.tracker_num):
                if self.global_time % 5 == 0:
                    self.update_tracker_goal(self.humans[i])
    ...
    def calc_reward(self, action, danger_zone):
        ...
        if self.global_time >= self.time_limit - 1:
            ...
            reward = self.goal_factor * (-abs(potential_cur) - self.potential)
